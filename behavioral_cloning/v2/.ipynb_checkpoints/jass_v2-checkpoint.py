...
Implements a manual training loop to control device placement of tensors
and allows specifying num_workers for DataLoader on CPU.
"""
...
# Need make_loader to create a PyTorch DataLoader from Transitions
# THIS IS USED IN THE MANUAL LOOP
from imitation.data import make_loader
...
# Import namedtuple from collections for optional metrics display
...
# This line will set device to 'cuda' if a GPU is available, otherwise 'cpu'
device = th.device("cuda" if th.cuda.is_available() else "cpu")
logging.info(f"Using device: {device}")


# --- Optional: Metrics structure for logging ---
# Define a simple named tuple to hold metrics during manual training loop
ManualTrainingMetrics = namedtuple("ManualTrainingMetrics", ["loss", "log_prob", "entropy", "accuracy"])
...
if __name__ == "__main__":
    ...
    # The Transitions object holds NumPy arrays. It does NOT handle devices.
    ...
    bc_trainer = bc.BC(
        ...
        # This will likely cause the device RuntimeError when device is 'cuda'
        # If you encounter the device RuntimeError again, you MUST use the manual
        # training loop script instead.
    )
